...
- Server logs show `Session <A> is not in the same call as session <B>, not requesting offer` for every `requestoffer`. At the same time the HPB logs backend ping errors such as `Could not decode OCS response body [] from https://cloud.codemyriad.io/ocs/v2.php/apps/spreed/api/v3/signaling/backend/ocs/v2.php/apps/spreed/api/v3/signaling/backend` and `Error sending combined ping ... /api/v1/signaling/backend`, i.e. the backend URL path is being double-appended. This likely prevents the HPB from learning “in call” state, so `isInSameCall` always fails and `requestoffer` is rejected.
- Tried enabling the escape hatch: set `allowsubscribeany = true` in `/conf/signaling.conf` on `nextcloud-aio-talk` and restarted the container (change logged in `~/dev/production_changes_diary.txt`). Behaviour did not change; `requestoffer` still returns `not_allowed`, so either the flag is ignored by this build or the in-call check fails earlier because backend pings are broken.
