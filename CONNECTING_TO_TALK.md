...
- Next step: capture the browser’s exact `requestoffer` frame (full JSON) via DevTools and compare fields; check server-side logs/HPB code for the gate that emits `not_allowed` on `requestoffer` to understand required prerequisites (clientType, call state, permissions).
